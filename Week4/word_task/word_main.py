...
def template1(name):
    ...
    p_customer.alignment = WD_PARAGRAPH_ALIGNMENT.LEFT  # 段落居中对齐
    # ----添加文字----
    ...
    # ----添加文字----
    run_text_3 = p3.add_run('       诚邀您参加由万门大学组织的客户答谢会。请于2020年10月1日在北京万门大学总部准时出席。')
...
def template3(name):
    ...
    p1.alignment = WD_PARAGRAPH_ALIGNMENT.CENTER  # 段落居中对齐
    # ----添加文字----
    run_text_1 = p1.add_run('万门大学客户大会') ####特备注意 这里是run修改某一个字的字体####
...
# #---特别注意 run是以一个字为颗粒度的添加
#     run_text_c = p_customer.add_run('尊敬的')
#     run_text_c.font.size = Pt(20)
#
#     run_text_c = p_customer.add_run('%s' % name)
#     run_text_c.font.size = Pt(20)
#     run_text_c.font.color.rgb = RGBColor(177, 23, 79)  # 注意这里是改变颜色的
#
#     run_text_c = p_customer.add_run('老师')
#     run_text_c.font.size = Pt(20)
# #---------------------------------
    # ----段落创建P3-----
    p3 = word_document.add_paragraph()  # 向word添加段落
...
#***** 以上为模板 以下为控制 读出数 然后 分模板 进行创建word
#主控
def main_creator():
